...
import { albumApi, imageApi, slideshowApi, getImageUrlWithAuth, type Album, type ImageItem, type Slideshow, type SlideshowWithImages, type TransitionEffect, type TextPosition, type PermissionLevel } from '@/lib/api';
...
export default function SlideshowEdit() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 尝试获取图片信息用于预览
      if (sl.images.length > 0 && albums.length > 0) {
        // 如果当前没有选中相册，查找包含第一张图片的相册
        if (!selectedAlbumId) {
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {albumImages.map(img => (
                ...
                >
                  <img
                    src={getImageUrlWithAuth(img.id)}
                    alt={img.name}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 flex flex-col gap-2">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={e => canEdit && updateSlide(idx, { overlayText: e.target.value })}
                            placeholder="叠加文字..."
                            disabled={!canEdit}
                          ...
                          <select
                            ...
                            onChange={e => canEdit && updateSlide(idx, { textPosition: e.target.value as TextPosition })}
                            disabled={!canEdit}
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={e => canEdit && updateSlide(idx, { textColor: e.target.value })}
                            disabled={!canEdit}
                          ...
                          <input
                            ...
                            onChange={e => canEdit && updateSlide(idx, { textSize: parseInt(e.target.value) || 16 })}
                            disabled={!canEdit}
                            min="12" max="48"
                      ...
                      <div className="flex flex-col justify-center gap-1">
                        <button
                          onClick={(e) => { e.stopPropagation(); canEdit && moveSlide(idx, -1); }}
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); canEdit && removeSlide(idx); }}
                          disabled={!canEdit}
        ...
        <div className="w-72 border-l border-white/5 p-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-3 gap-2">
            {EFFECTS.map(effectName => (
              <button
                key={effectName}
                onClick={() => canEdit && setEffect(effectName)}
                disabled={!canEdit}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent px-2 py-1">
                  <span className="text-xs text-white">{effectLabels[effectName]}</span>
                </div>
              </button>
            ))}
          </div>
          ...
          <div className="mt-6 space-y-4">
            <div>
              <label className="flex items-center justify-between text-sm mb-2">
                <span>间隔时间</span>
                <span className="text-[#F5F0EB]/50">{interval} 秒</span>
              </label>
              <input
                type="range"
                min="1" max="10"
                value={interval}
                onChange={e => canEdit && setInterval(parseInt(e.target.value))}
                disabled={!canEdit}
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => canEdit && setAutoPlay(!autoPlay)}
                disabled={!canEdit}
